      ...
      <div className="grid grid-cols-1 md:grid-cols-3 md:grid-rows-3 gap-6 h-full">
        {FEATURES.map((feature, i) => (
          <motion.div
            ...
            viewport={{ once: true }}
            className={cn(
              "relative group overflow-hidden rounded-3xl border border-white/5 bg-background text-foreground p-10 flex flex-col justify-between hover:border-emerald-500/30 transition-all duration-500 shadow-xl",
              feature.className
            )}
          >
            {/* Asset Background */}
            <div className="absolute top-0 right-0 w-64 h-64 -mr-20 -mt-20 opacity-10 group-hover:opacity-20 transition-opacity duration-500">
               <feature.asset color={feature.color as "emerald" | "blue"} />
            </div>

            <div className="relative z-10 space-y-6">
              <div className={cn(
                  "h-14 w-14 rounded-2xl border flex items-center justify-center shadow-lg",
                  feature.color === "emerald" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-400" : "border-blue-500/30 bg-blue-500/10 text-blue-400"
              )}>
                  <feature.asset className="w-10 h-10" color={feature.color as "emerald" | "blue"} />
              </div>
            ...
            <div className="mt-10 flex items-center gap-2 text-[10px] font-mono text-muted-foreground uppercase tracking-widest footer-tag">
              <span className={cn("h-2 w-2 rounded-full", feature.color === "emerald" ? "bg-emerald-500" : "bg-blue-500")} />
              <Badge variant="ghost" className="text-muted-foreground px-0">System_Component // {feature.color}</Badge>
            </div>
          </motion.div>
        ))}
      </div>
