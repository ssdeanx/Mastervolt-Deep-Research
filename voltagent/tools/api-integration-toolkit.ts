...
export const fetchApiTool = createTool({
    ...
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { url: string; method: string }
            voltlogger.info('fetch_api: start', {
                tool: tool.name,
        ...
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('fetch_api: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
    ...
    execute: async (args, context?: ToolExecuteOptions) => {
        ...
        voltlogger.info(`API fetch: ${args.method} ${args.url}`, {
                ...
                try {
                    if (typeof responseRaw === 'symbol') {
                        ...
                        try {
                            ...
                            responseData =
                                json ?? inspect(responseRaw, { depth: null })
                        } catch {
...
export const aggregateApisTool = createTool({
    ...
    tags: ['api', 'aggregation', 'batch'],
    parameters: z.object({
        endpoints: z.array(z.object({ url: z.string(), name: z.string() })),
        mergeStrategy: z.enum(['concat', 'merge', 'dedupe']).default('concat'),
    }),
    outputSchema: z.object({
        results: z.array(
            z.object({
        ...
        mergeStrategy: z.enum(['concat', 'merge', 'dedupe']),
        timestamp: z.iso.datetime(),
    }),
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            ...
            voltlogger.info('aggregate_apis: start', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
        ...
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('aggregate_apis: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
            ...
            voltlogger.info('aggregate_apis: end', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
            })
    ...
    execute: async (args, context?: ToolExecuteOptions) => {
        ...
        voltlogger.info(`Aggregating ${args.endpoints.length} APIs`, {
            operationId: context?.operationId,
        })
            ...
            for (const endpoint of args.endpoints) {
                try {
                    const response: AxiosResponse<unknown> =
                        await axios.get<unknown>(endpoint.url)
                    results.push({
                        name: endpoint.name,
                        data: response.data,
                        success: true,
...
export const handlePaginationTool = createTool({
    ...
    parameters: z.object({
        baseUrl: z.string(),
        paginationType: z.enum(['offset', 'cursor', 'page']).default('page'),
    ...
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { baseUrl: string; paginationType: string }
            voltlogger.info('handle_pagination: start', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
        ...
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('handle_pagination: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
            ...
            voltlogger.info('handle_pagination: end', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
            })
    ...
    execute: async (args, context?: ToolExecuteOptions) => {
        ...
        voltlogger.info(`Handling pagination: ${args.paginationType}`, {
            operationId: context?.operationId,
        })
        ...
        try {
            ...
            for (let page = 1; page <= args.maxPages; page++) {
                ...
                if (status >= 200 && status < 300) {
                    const resultsToAdd = Array.isArray(data)
                        ? (data as unknown[])
...
export const cacheApiResponseTool = createTool({
    ...
    parameters: z.object({
        ...
        action: z.enum(['get', 'set', 'invalidate']).default('get'),
    }),
    outputSchema: z.object({
        success: z.boolean(),
    ...
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { key: string; action: string }
            voltlogger.info('cache_api_response: start', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
        ...
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('cache_api_response: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
            ...
            voltlogger.info('cache_api_response: end', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
            })
    ...
    execute: (args, context?: ToolExecuteOptions) => {
        ...
        voltlogger.info(`Cache ${args.action}: ${args.key}`, {
            operationId: context?.operationId,
        })
        ...
        try {
            switch (args.action) {
                ...
                default:
                    return {
                        success: false,
                        action: args.action as 'get' | 'set' | 'invalidate',
                        cached: false,
...
export const retryWithBackoffTool = createTool({
    ...
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { url: string; maxRetries: number }
            voltlogger.info('retry_with_backoff: start', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
                url: params.url,
        ...
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('retry_with_backoff: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
            ...
            voltlogger.info('retry_with_backoff: end', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
            })
    ...
    execute: async (args, context?: ToolExecuteOptions) => {
        ...
        voltlogger.info(`Retry with backoff: ${args.url}`, {
            operationId: context?.operationId,
        })
        ...
        for (let attempt = 0; attempt <= args.maxRetries; attempt++) {
            ...
            try {
                const delay = Math.min(
                    args.initialDelayMs * Math.pow(2, attempt),
                    args.maxDelayMs
                )
                if (attempt > 0) {
                    voltlogger.info(
                        `Retry attempt ${attempts} after ${delay}ms delay`
                    )
                    await new Promise((resolve) => setTimeout(resolve, delay))
                }

                const response: AxiosResponse<unknown> =
                    await axios.get<unknown>(args.url)
                return {
                    success: true,
                    data: response.data,
                    attempts,
...
export const normalizeApiResponseTool = createTool({
    ...
    tags: ['api', 'normalization', 'data-transformation'],
    parameters: z.object({
        response: z.unknown(),
        schema: z.record(z.string(), z.string()).describe('Field mapping'),
    }),
    outputSchema: z.object({
        normalized: z.record(z.string(), z.unknown()),
        originalFields: z.array(z.string()),
    ...
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { schema: Record<string, string> }
            voltlogger.info('normalize_api_response: start', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
        ...
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('normalize_api_response: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
            ...
            voltlogger.info('normalize_api_response: end', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
            })
    ...
    execute: (args, context) => {
        ...
        voltlogger.info('Normalizing API response', {
            operationId: context?.operationId,
        })
...
export const apiIntegrationToolkit: Toolkit = createToolkit({
    ...
    instructions:
        'Use these tools for API calls, pagination, caching, and response normalization.',
    addInstructions: true,
    tools: [
        fetchApiTool,
        aggregateApisTool,
        handlePaginationTool,
        cacheApiResponseTool,
        retryWithBackoffTool,
        normalizeApiResponseTool,
    ] as unknown as Tool[],
})
