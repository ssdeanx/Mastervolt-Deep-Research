...
export const csvToJsonTool = createTool({
  ...
  description:
    'Convert CSV data to JSON format for visualization and analysis',
  parameters: z.object({
  ...
  execute: (args, context) => {
    ...
    try {
      ...
      voltlogger.info(
        `Converted CSV with ${jsonData.length} rows and ${tableData.headers.length} columns`
      )
      ...
      voltlogger.error(`CSV conversion failed: ${String(error)}`)
...
export const xmlToJsonTool = createTool({
  ...
  parameters: z.object({
    ...
    ignoreAttributes: z
      .boolean()
      .default(false)
      .describe('Whether to ignore XML attributes'),
    attributeNamePrefix: z
      .string()
      .default('@_')
      .describe('Prefix for attribute names'),
    parseAttributeValue: z
      .boolean()
      .default(true)
      .describe('Whether to parse attribute values'),
  }),
  execute: (args, context) => {
    ...
    try {
      ...
      const jsonData: unknown = parser.parse(args.xmlData)
      ...
      const getStructureInfo = (obj: unknown, depth = 0): unknown => {
        ...
        if (Array.isArray(obj)) {
          return {
            ...
            items:
              obj.length > 0
                ? getStructureInfo(obj[0], depth + 1)
                : null,
          }
        }
      ...
      return {
        ...
        rootKeys:
          typeof jsonData === 'object' && jsonData !== null
            ? Object.keys(jsonData as Record<string, unknown>)
            : [],
        dataType: Array.isArray(jsonData) ? 'array' : typeof jsonData,
      }
    } catch (error) {
      voltlogger.error(`XML conversion failed: ${String(error)}`)
      throw new Error(`Failed to convert XML to JSON: ${String(error)}`)
    }
  },
})
...
export const jsonToSvgTool = createTool({
  ...
  description:
    'Create SVG visualization from JSON data - the main visualization tool',
  parameters: z.object({
    jsonData: z
      .array(z.record(z.string(), z.union([z.string(), z.number()])))
      .describe('Array of JSON objects to visualize'),
    chartType: z
  ...
  execute: (args, context) => {
    ...
    try {
      ...
      const labels = args.jsonData.map((row) =>
        String(row[args.labelField] || '')
      )
      ...
      switch (args.chartType) {
        ...
        case 'pie': {
          ...
          values.forEach((value, index) => {
            ...
            const x2 =
              centerX + Math.cos(currentAngle + angle) * radius
            const y2 =
              centerY + Math.sin(currentAngle + angle) * radius
      ...
      voltlogger.info(
        `Created ${args.chartType} SVG with ${values.length} data points`
      )
      ...
      voltlogger.error(`SVG creation failed: ${String(error)}`)
      throw new Error(`Failed to create SVG: ${String(error)}`)
    }
  },
})
...
export const svgToJsonTool = createTool({
  ...
  description:
    'Convert SVG data to JSON format for analysis and manipulation',
  parameters: z.object({
    svgData: z.string().describe('SVG data as a string'),
    extractPaths: z
      .boolean()
      .default(true)
      .describe('Whether to extract path elements'),
    extractStyles: z
      .boolean()
      .default(true)
      .describe('Whether to extract style information'),
  }),
  execute: (args, context) => {
    if (!context?.isActive) {
      throw new Error('Operation has been cancelled')
    ...
    try {
      ...
      const parsedSvg = parser.parse(args.svgData) as Record<
        string,
        unknown
      >
      const svgRoot = parsedSvg.svg ?? parsedSvg

      // Safely parse numeric width/height attributes. Avoid stringifying objects and handle
      // undefined/null values explicitly. Use explicit type checks to satisfy the linter.
      const parseSvgDimension = (value: unknown): number => {
        ...
        if (typeof value === 'string') {
          ...
          // Try parsing integer first, then fall back to float parsing for values like "100.5"
          const intParsed = parseInt(trimmed, 10)
          if (!Number.isNaN(intParsed)) {
            return intParsed
          }
          const floatParsed = parseFloat(trimmed)
          return Number.isNaN(floatParsed)
            ? 0
            : Math.floor(floatParsed)
        }
        return 0
      }

      const svgData: SVGData = {
        width: parseSvgDimension(
          (svgRoot as Record<string, unknown>)['@_width']
        ),
        height: parseSvgDimension(
          (svgRoot as Record<string, unknown>)['@_height']
        ),
        elements: [],
      }
      ...
      if (args.extractPaths && jsonResult) {
        const extractElements = (obj: unknown): void => {
          ...
          const MAX_ATTR_STRING_LENGTH = 200
          ...
          Object.entries(obj as Record<string, unknown>).forEach(
            ([key, value]) => {
              if (key.startsWith('@_')) {
                // This is an attribute on the parent, skip here
                return
              }

              // Helper to process a single element instance (handles objects produced by parser)
              const processInstance = (instance: unknown) => {
                if (
                ...
                // Extract attributes and textual content
                Object.entries(
                  instance as Record<string, unknown>
                ).forEach(([attrKey, attrValue]) => {
                  if (attrKey.startsWith('@_')) {
                    const name = attrKey.substring(2)
                    ...
                    if (
                      ...
                      // Convert boolean to string for attribute storage
                      element.attributes[name] = attrValue
                        ? 'true'
                        : 'false'
                    } else if (
                      ...
                      // For objects/arrays/other types, JSON.stringify safely (fallback on error),
                      // and truncate to avoid extremely long attribute strings.
                      try {
                        const serialized =
                          typeof attrValue ===
                            'string'
                    ...
                    // Non-attribute entries might be text content (#text or similar)
                    if (
                      typeof attrValue === 'string' ||
                      typeof attrValue === 'number'
                    ) {
                      const text =
                        String(attrValue).trim()
                      if (text !== '') {
                ...
                // Recurse into nested elements (skip attributes)
                Object.entries(
                  instance as Record<string, unknown>
                ).forEach(([childKey, childValue]) => {
                  if (!childKey.startsWith('@_')) {
                    extractElements(childValue)
                  }
                })
              }

              // value can be an array (multiple elements), an object, or directly textual content
              if (Array.isArray(value)) {
                value.forEach((item) => processInstance(item))
              } else if (
                typeof value === 'object' &&
                value !== null
              ) {
                ...
                // Direct text node for this key
                svgData.elements.push({
                  type: key,
                  attributes: {},
      ...
      // Determine if any element has style information. Coerce to a boolean explicitly to satisfy the type checker.
      const hasStyles =
        args.extractStyles &&
        svgData.elements.some((el) => {
          const s = el.attributes.style
          if (typeof s === 'string') {
            return s.trim() !== ''
          }
          if (typeof s === 'number') {
      ...
      voltlogger.info(
        `Converted SVG with ${svgData.elements.length} elements`
      )
      ...
      voltlogger.error(`SVG conversion failed: ${String(error)}`)
      throw new Error(`Failed to convert SVG to JSON: ${String(error)}`)
    }
  },
})
  ...
  parameters: z.object({
    excalidrawData: z
      .object({
        elements: z
          .array(z.any())
          .describe('Excalidraw elements array'),
        appState: z
          .object({
    ...
    exportPadding: z
      .number()
      .default(10)
      .describe('Padding around the exported SVG'),
  }),
  execute: (args, context) => {
    if (!context || !(context.isActive ?? false)) {
      throw new Error('Operation has been cancelled')
    ...
    try {
      const svg = excalidrawToSvg(args.excalidrawData, {
        exportPadding: args.exportPadding,
      })

      voltlogger.info(
        `Successfully converted Excalidraw with ${args.excalidrawData.elements.length} elements to SVG`
      )

      return {
        svg,
        elementCount: args.excalidrawData.elements.length,
        hasFiles: args.excalidrawData.files
          ? Object.keys(args.excalidrawData.files).length > 0
          : false,
      }
    } catch (error) {
      voltlogger.error(
        `Excalidraw to SVG conversion failed: ${String(error)}`
      )
      throw new Error(
        `Failed to convert Excalidraw to SVG: ${String(error)}`
      )
    }
  },
})
...
export const createCustomSvgTool = createTool({
  ...
  parameters: z.object({
    ...
    elements: z
      .array(
        z.object({
          type: z
            .enum(['rect', 'circle', 'line', 'text', 'path'])
  ...
  execute: (args, context) => {
    ...
    voltlogger.info(
      `Creating custom SVG with ${args.elements.length} elements`
    )
    ...
    try {
      ...
      if (args.backgroundColor && args.backgroundColor.trim() !== '') {
      ...
      voltlogger.info(
        `Created custom SVG with ${args.elements.length} elements`
      )
      ...
      voltlogger.error(`Custom SVG creation failed: ${String(error)}`)
      throw new Error(`Failed to create custom SVG: ${String(error)}`)
    }
  },
})
...
export const createChartDataTool = createTool({
  ...
  description:
    'Create chart-ready data structure from JSON data for visualization',
  parameters: z.object({
    jsonData: z
      .array(z.record(z.string(), z.union([z.string(), z.number()])))
      .describe('Array of JSON objects'),
    labelField: z.string().describe('Field name to use as labels'),
    valueFields: z
      .array(z.string())
      .describe('Field names to use as data values'),
    chartType: z
      .enum(['bar', 'line', 'pie', 'scatter'])
      .default('bar')
      .describe('Type of chart to create data for'),
  }),
  execute: (args, context) => {
    if (!context || !context.isActive) {
      throw new Error('Operation has been cancelled')
    ...
    try {
      ...
      voltlogger.info(
        `Created chart data with ${labels.length} labels and ${datasets.length} datasets`
      )
      ...
      voltlogger.error(`Chart data creation failed: ${String(error)}`)
      throw new Error(`Failed to create chart data: ${String(error)}`)
    }
  },
})
...
export const visualizationToolkit = createToolkit({
  ...
  description:
    'Comprehensive visualization toolkit for creating SVG charts and graphics from data.',
