...
const http = axios.create({
    ...
    headers: {
        'User-Agent':
            'Mastervolt-Deep-Research/1.0 (+https://github.com/ssdeanx/Mastervolt-Deep-Research)',
    },
})
const retryDelay = axiosRetry.exponentialDelay.bind(axiosRetry)
const retryCondition =
    axiosRetry.isNetworkOrIdempotentRequestError.bind(axiosRetry)
...
function transformXmlIfDetected(text: string): {
    isXml: boolean
    text: string
} {
    ...
    if (
        /^<\?xml/i.test(trimmed) ||
        /<\s*(rss|feed|rdf|opml)(\s|>)/i.test(trimmed)
    ) {
        try {
    ...
    tools: [
        createTool({
            ...
            parameters: z.object({
                ...
                removeScripts: z
                    ...
                    .default(true)
                    .describe(
                        'Remove script tags and navigation elements (default: true)'
                    ),
                includeImages: z
                    .boolean()
                    .optional()
                    .default(true)
                    .describe(
                        'Include image markdown with alt text (default: true)'
                    ),
                maxLength: z
                    .number()
                    .positive()
                    .optional()
                    .describe(
                        'Maximum length of content in characters. If exceeded, content will be truncated.'
                    ),
            }),
            ...
            hooks: {
                onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
                    ...
                    voltlogger.info('scrape_webpage_markdown: start', {
                        tool: tool.name,
                ...
                onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
                    if (error !== undefined && error !== null) {
                        voltlogger.error('scrape_webpage_markdown: error', {
                            tool: tool.name,
                            operationId: options?.operationId,
                            toolCallId: options?.toolContext?.callId,
                            error:
                    ...
                    voltlogger.info('scrape_webpage_markdown: end', {
                        tool: tool.name,
                        operationId: options?.operationId,
                        toolCallId: options?.toolContext?.callId,
                    })
            ...
            execute: async (args, context?: ToolExecuteOptions) => {
                ...
                try {
                    ...
                    const { isXml, text: html } =
                        transformXmlIfDetected(htmlRaw)
                    if (isXml) {
                        voltlogger.info(
                            `Detected XML at ${args.url} and converted to JSON representation`,
                            { operationId: context?.operationId }
                        )
                    }
                    ...
                    if (
                        ...
                        Number.isFinite(args.maxLength) &&
                        args.maxLength > 0 &&
                        finalMarkdown.length > args.maxLength
                    ) {
                        finalMarkdown =
                            finalMarkdown.substring(0, args.maxLength) +
                            '\n\n[... content truncated ...]'
                    }

                    voltlogger.info(
                        `Successfully scraped webpage: ${args.url}`,
                        {
                            operationId: context?.operationId,
                    ...
                    voltlogger.error(
                        `Webpage scraping failed: ${errorMessage}`,
                        {
                            operationId: context?.operationId,
                            url: args.url,
                        }
        ...
        createTool({
            ...
            parameters: z.object({
                url: z
                    .url()
                    .describe(
                        'The URL of the webpage to extract code blocks from'
                    ),
                includeContext: z
                    .boolean()
                    .optional()
                    .default(true)
                    .describe(
                        'Include surrounding text context for each code block (default: true)'
                    ),
                contextLines: z
                    .number()
                    .min(1)
                    ...
                    .default(3)
                    .describe(
                        'Number of lines before and after each code block to include as context'
                    ),
            }),
            outputSchema: z.object({
                url: z.url(),
                totalCodeBlocks: z.number(),
                codeBlocks: z.array(
                    z.object({
                        language: z.string(),
                        code: z.string(),
            ...
            hooks: {
                onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
                    const params = args as { url: string }
                    voltlogger.info('extract_code_blocks: start', {
                        tool: tool.name,
                        operationId: options?.operationId,
                        toolCallId: options?.toolContext?.callId,
                        url: params.url,
                ...
                onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
                    if (error !== undefined && error !== null) {
                        voltlogger.error('extract_code_blocks: error', {
                            tool: tool.name,
                            operationId: options?.operationId,
                            toolCallId: options?.toolContext?.callId,
                            error:
                    ...
                    voltlogger.info('extract_code_blocks: end', {
                        tool: tool.name,
                        operationId: options?.operationId,
                        toolCallId: options?.toolContext?.callId,
                    })
            ...
            execute: async (args, context?: ToolExecuteOptions) => {
                ...
                try {
                    voltlogger.info(
                        `Extracting code blocks from: ${args.url}`,
                        {
                            operationId: context?.operationId,
                        }
                    )
                    ...
                    $('pre, code').each((idx, element) => {
                        ...
                        if (code.trim()) {
                            ...
                            if (args.includeContext) {
                                ...
                                voltlogger.debug(
                                    `Context collected: ${collectedBefore} before, ${collectedAfter} after lines`,
                                    {
                                        operationId: context?.operationId,
                            ...
                            if (
                                typeof filenameAttr === 'string' &&
                                filenameAttr.length > 0
                            ) {
                    ...
                    voltlogger.info(
                        `Extracted ${codeBlocks.length} code blocks`,
                        {
                            operationId: context?.operationId,
                        }
                    )
                    ...
                    const errorMessage =
                        error instanceof Error ? error.message : String(error)
                    voltlogger.error(
                        `Code block extraction failed: ${errorMessage}`,
                        {
                            operationId: context?.operationId,
                            url: args.url,
                        }
                    ...
                    throw new Error(
                        `Failed to extract code blocks: ${errorMessage}`
                    )
                }
            },
        }),
        createTool({
            ...
            parameters: z.object({
                ...
                includeMetadata: z
                    ...
                    .default(true)
                    .describe(
                        'Include page metadata (title, description, keywords) (default: true)'
                    ),
                includeLinks: z
                    .boolean()
                    .optional()
                    .default(true)
                    .describe(
                        'Include all links from the page (default: true)'
                    ),
                includeTables: z
                    .boolean()
                    .optional()
                    .default(true)
                    .describe(
                        'Extract table data in structured format (default: true)'
                    ),
            }),
            outputSchema: z.object({
                url: z.url(),
            ...
            hooks: {
                onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
                    const params = args as { url: string }
                    voltlogger.info('extract_structured_data: start', {
                        tool: tool.name,
                        operationId: options?.operationId,
                        toolCallId: options?.toolContext?.callId,
                        url: params.url,
                ...
                onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
                    if (error !== undefined && error !== null) {
                        voltlogger.error('extract_structured_data: error', {
                            tool: tool.name,
                            operationId: options?.operationId,
                            toolCallId: options?.toolContext?.callId,
                            error:
                    ...
                    voltlogger.info('extract_structured_data: end', {
                        tool: tool.name,
                        operationId: options?.operationId,
                        toolCallId: options?.toolContext?.callId,
                    })
            ...
            execute: async (args, context?: ToolExecuteOptions) => {
                ...
                try {
                    voltlogger.info(
                        `Extracting structured data from: ${args.url}`,
                        {
                            operationId: context?.operationId,
                        }
                    )
                    ...
                    if (args.includeTables) {
                        ...
                        $('table').each((idx, el) => {
                            ...
                            $table
                                .find('thead tr th')
                                .each((headerIndex, cell) => {
                                    headers.push($(cell).text().trim())
                                    // Validate header index matches expected position
                                    if (headerIndex >= headers.length) {
                                        voltlogger.warn(
                                            `Header index mismatch at position ${headerIndex}`,
                                            {
                                                operationId:
                                                    context?.operationId,
                                            }
                            ...
                            $table.find('tbody tr').each((rowIndex, row) => {
                                ...
                                $row.find('td').each((cellIndex, cell) => {
                                    ...
                                    if (cellIndex >= rowData.length) {
                                        voltlogger.warn(
                                            `Cell index mismatch in row ${rowIndex}, cell ${cellIndex}`,
                                            {
                                                operationId:
                                                    context?.operationId,
                                            }
                                ...
                                if (rowData.length > 0) {
                                    ...
                                    if (
                                        rows.length > 1 &&
                                        rowData.length !==
                                            rows[rows.length - 2].length
                                    ) {
                                        voltlogger.warn(
                                            `Inconsistent row length at row ${rowIndex}: expected ${rows[rows.length - 2].length}, got ${rowData.length}`,
                                            {
                                                operationId:
                                                    context?.operationId,
                                            }
                            ...
                            .each((itemIndex, li) => {
                                ...
                                if (itemText) {
                                    ...
                                    if (itemIndex !== items.length - 1) {
                                        voltlogger.warn(
                                            `Ordered list item index mismatch: expected ${items.length - 1}, got ${itemIndex}`,
                                            {
                                                operationId:
                                                    context?.operationId,
                                            }
                        ...
                        $(el)
                            ...
                            .each((itemIndex, li) => {
                                ...
                                if (itemText) {
                                    ...
                                    if (itemIndex !== items.length - 1) {
                                        voltlogger.warn(
                                            `Unordered list item index mismatch: expected ${items.length - 1}, got ${itemIndex}`,
                                            {
                                                operationId:
                                                    context?.operationId,
                                            }
                    ...
                    voltlogger.info(
                        `Extracted structured data from: ${args.url}`,
                        {
                            operationId: context?.operationId,
                    ...
                    const errorMessage =
                        error instanceof Error ? error.message : String(error)
                    voltlogger.error(
                        `Structured data extraction failed: ${errorMessage}`,
                        {
                            operationId: context?.operationId,
                            url: args.url,
                        }
        ...
        createTool({
            ...
            parameters: z.object({
                url: z
                    .url()
                    .describe('The URL of the webpage to extract text from'),
                removeNavigation: z
                    .boolean()
                    .optional()
                    .default(true)
                ...
                cleanWhitespace: z
                    ...
                    .default(true)
                    .describe(
                        'Clean excessive whitespace and newlines (default: true)'
                    ),
            }),
            outputSchema: z.object({
                url: z.url(),
            ...
            hooks: {
                onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
                    const params = args as { url: string }
                    voltlogger.info('extract_text_content: start', {
                        tool: tool.name,
                        operationId: options?.operationId,
                        toolCallId: options?.toolContext?.callId,
                        url: params.url,
                ...
                onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
                    if (error !== undefined && error !== null) {
                        voltlogger.error('extract_text_content: error', {
                            tool: tool.name,
                            operationId: options?.operationId,
                            toolCallId: options?.toolContext?.callId,
                            error:
                    ...
                    voltlogger.info('extract_text_content: end', {
                        tool: tool.name,
                        operationId: options?.operationId,
                        toolCallId: options?.toolContext?.callId,
                    })
            ...
            execute: async (args, context?: ToolExecuteOptions) => {
                ...
                try {
                    voltlogger.info(
                        `Extracting text content from: ${args.url}`,
                        {
                            operationId: context?.operationId,
                        }
                    )
                    ...
                    if (args.removeNavigation) {
                        document
                            .querySelectorAll('nav, footer, header')
                            .forEach((el) => {
                                el.remove()
                            })
                    }
                    ...
                    voltlogger.info(
                        `Successfully extracted text from: ${args.url}`,
                        {
                            operationId: context?.operationId,
                    ...
                    throw new Error(
                        `Failed to extract text content: ${errorMessage}`
                    )
                }
            },
        }),
        createTool({
            ...
            parameters: z.object({
                urls: z
                    ...
                    .min(1)
                    .max(50)
                    .describe('Array of starting URLs to scrape (max 50)'),
                maxDepth: z
                    .number()
                    .min(1)
                    .max(5)
                    ...
                    .default(1)
                    .describe(
                        'Maximum depth to follow links (1 = only given URLs, 2 = follow links from given URLs, etc.)'
                    ),
                maxPages: z
                    .number()
                    .min(1)
                    ...
                    .default(10)
                    .describe(
                        'Maximum total pages to scrape across all URLs and depths'
                    ),
                ...
                includePatterns: z
                    ...
                    .optional()
                    .describe(
                        'Regex patterns for URLs to include (if specified, only matching URLs will be scraped)'
                    ),
                excludePatterns: z
                    .array(z.string())
                    .optional()
                    .describe(
                        'Regex patterns for URLs to exclude from scraping'
                    ),
            }),
            outputSchema: z.object({
                startingUrls: z.array(z.url()),
                totalPagesAttempted: z.number(),
                ...
                batchCompletedAt: z.iso.datetime(),
            }),
            hooks: {
                onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
                    const params = args as { urls: string[] }
                    voltlogger.info('batch_scrape_pages: start', {
                        tool: tool.name,
                        operationId: options?.operationId,
                        toolCallId: options?.toolContext?.callId,
                ...
                onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
                    if (error !== undefined && error !== null) {
                        voltlogger.error('batch_scrape_pages: error', {
                            tool: tool.name,
                            operationId: options?.operationId,
                            toolCallId: options?.toolContext?.callId,
                            error:
                    ...
                    voltlogger.info('batch_scrape_pages: end', {
                        tool: tool.name,
                        operationId: options?.operationId,
                        toolCallId: options?.toolContext?.callId,
                    })
            ...
            execute: async (args, context?: ToolExecuteOptions) => {
                ...
                args.urls.forEach((url) => ensureHttpUrl(url))

                try {
                    voltlogger.info(
                        `Starting batch scrape: ${args.urls.length} URLs, depth ${args.maxDepth}`,
                        {
                            operationId: context?.operationId,
                        }
                    )
                    ...
                    while (
                        ...
                        try {
                            ...
                            result.error =
                                error instanceof Error
                                    ...
                                    : String(error)
                            voltlogger.warn(
                                `Failed to scrape ${url}: ${result.error}`,
                                {
                                    operationId: context?.operationId,
                                }
                            )
                        ...
                        if (urlsToVisit.length > 0 && context?.isActive) {
                            await new Promise((resolve) =>
                                setTimeout(resolve, args.delayMs)
                            )
                        }
                    }
                    ...
                    const failedScrapes = results.filter(
                        (r) => !r.success
                    ).length

                    voltlogger.info(
                        `Batch scraping completed: ${successfulScrapes} successful, ${failedScrapes} failed`,
                        {
                            operationId: context?.operationId,
                    ...
                    throw new Error(
                        `Failed to perform batch scraping: ${errorMessage}`
                    )
                }
            },
        }),
