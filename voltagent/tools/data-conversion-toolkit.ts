...
const safeStringify = (value: unknown): string => {
    ...
    try {
        return (
            JSON.stringify(value, (_k: string, v: unknown) => {
...
export const csvToJsonTool = createTool({
    name: 'csv_to_json',
    description:
        'Convert CSV data to JSON format. Supports custom delimiters and field mapping.',
    ...
    parameters: z.object({
        ...
        delimiter: z
            .string()
            .default(',')
            .describe('CSV delimiter character (comma, semicolon, tab, etc.)'),
        fieldDelimiter: z
            .string()
            .optional()
            .describe('Field delimiter if different from main delimiter'),
        skipHeader: z
            .boolean()
            .default(false)
            .describe('Skip the first row if it contains headers'),
        headers: z
            .array(z.string())
            .optional()
            .describe('Custom headers to use instead of first row'),
    ...
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { csvData: string; delimiter: string }
            voltlogger.info('csv_to_json: start', {
                tool: tool.name,
        ...
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('csv_to_json: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
            ...
            voltlogger.info('csv_to_json: end', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
            })
    ...
    execute: (args, context?: ToolExecuteOptions) => {
        ...
        voltlogger.info(
            `Converting CSV to JSON (${args.csvData.length} characters)`,
        ...
        try {
            ...
            if (args.delimiter === ',') {
                ...
                const raw = ConvertCsvToJson.csvStringToJson(
                    args.csvData
                ) as unknown
            ...
            return {
                ...
                headers:
                    args.headers ?? (result[0] ? Object.keys(result[0]) : []),
...
export const jsonToCsvTool = createTool({
    name: 'json_to_csv',
    description:
        'Convert JSON data to CSV format. Supports custom delimiters and field selection.',
    ...
    parameters: z.object({
        ...
        headers: z
            .array(z.string())
            .optional()
            .describe('Custom headers (uses object keys if not provided)'),
        includeHeaders: z
            .boolean()
            .default(true)
            .describe('Include headers in output'),
    }),
    outputSchema: z.object({
        csv: z.string(),
        rowCount: z.number(),
    ...
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { jsonData: unknown[]; delimiter: string }
            voltlogger.info('json_to_csv: start', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
                recordCount: params.jsonData.length,
                delimiter: params.delimiter,
            })
        },
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('json_to_csv: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
            ...
            voltlogger.info('json_to_csv: end', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
            })
    ...
    execute: (args, context?: ToolExecuteOptions) => {
        ...
        voltlogger.info(
            `Converting JSON to CSV (${args.jsonData.length} objects)`,
            { operationId: context?.operationId }
        )
        ...
        try {
            if (args.jsonData.length === 0) {
                return {
                    csv:
                        args.includeHeaders && args.headers
                            ? args.headers.join(args.delimiter)
                            : '',
                    rowCount: 0,
            ...
            const rows = args.jsonData.map((obj) => {
                return headers
                    .map((header) => {
                        ...
                        if (
                            stringValue.includes(args.delimiter) ||
...
export const xmlParseTool = createTool({
    name: 'xml_parse',
    description:
        'Parse XML data into JavaScript objects. Supports various XML formats and parsing options.',
    tags: ['conversion', 'xml', 'parse'],
    parameters: z.object({
        ...
        textNodeName: z
            .string()
            .default('#text')
            .describe('Name for text nodes'),
        ignoreNameSpace: z
            .boolean()
            .default(false)
            .describe('Ignore XML namespaces'),
        allowBooleanAttributes: z
            .boolean()
            .default(false)
            .describe('Allow boolean attributes'),
        parseNodeValue: z.boolean().default(true).describe('Parse node values'),
        parseAttributeValue: z
            .boolean()
            .default(true)
            .describe('Parse attribute values'),
    }),
    outputSchema: z.object({
        data: z.unknown(),
        options: z.object({
            ignoreAttributes: z.boolean(),
            attributeNamePrefix: z.string(),
            textNodeName: z.string(),
        }),
        timestamp: z.iso.datetime(),
    }),
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { xmlData: string }
            voltlogger.info('xml_parse: start', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
                dataLength: params.xmlData.length,
            })
        },
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('xml_parse: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
...
export const xmlBuildTool = createTool({
    name: 'xml_build',
    description:
        'Convert JavaScript objects to XML format. Supports custom formatting options.',
    tags: ['conversion', 'xml', 'build'],
    parameters: z.object({
        jsonData: z
            .record(z.string(), z.any())
            .describe('JavaScript object to convert to XML'),
        format: z
            .boolean()
            .default(true)
            .describe('Pretty format the XML output'),
        indentBy: z
            .string()
            .default('  ')
            .describe('Indentation string for pretty formatting'),
        suppressEmptyNode: z
            .boolean()
            .default(false)
            .describe('Suppress empty nodes'),
        suppressBooleanAttributes: z
            .boolean()
            .default(false)
            .describe('Suppress boolean attributes'),
        ignoreAttributes: z
            .boolean()
            .default(false)
            .describe('Ignore attributes during building'),
        attributeNamePrefix: z
            .string()
            .default('@_')
            .describe('Prefix for attribute names'),
    ...
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { jsonData: Record<string, unknown> }
            voltlogger.info('xml_build: start', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
                rootKeys: Object.keys(params.jsonData),
            })
        },
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('xml_build: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
...
export const dataConversionToolkit = createToolkit({
    ...
    description:
        'Tools for converting between different data formats including CSV, JSON, and XML.',
