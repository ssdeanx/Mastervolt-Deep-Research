...
export const getWeatherTool = createTool({
    ...
    parameters: z.object({
        location: z
            .string()
            .describe(
                "The location to get the weather for (e.g. 'London', 'New York')"
            ),
    }),
    ...
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { location: string }
            voltlogger.info('get_weather: start', {
                tool: tool.name,
        ...
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('get_weather: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
            ...
            voltlogger.info('get_weather: end', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
            })
    ...
    execute: async (args, context?: ToolExecuteOptions) => {
        ...
        try {
            ...
            voltlogger.error(
                `get_weather failed for ${location}: ${formatError(error)}`
            )
            return {
                success: false,
...
export const getForecastOpenMeteo = createTool({
    ...
    description:
        'Get structured forecast (hourly/daily) for a location via Nominatim + Open-Meteo (no API key)',
    ...
    parameters: z.object({
        ...
        days: z
            ...
            .max(16)
            .default(3)
            .describe('How many days to include (max 16)'),
    ...
    hooks: {
        onStart: ({ tool, args, options }: ToolHookOnStartArgs) => {
            const params = args as { location: string }
            voltlogger.info('get_forecast_open_meteo: start', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
                location: params.location,
        ...
        onEnd: ({ tool, error, options }: ToolHookOnEndArgs) => {
            if (error !== undefined && error !== null) {
                voltlogger.error('get_forecast_open_meteo: error', {
                    tool: tool.name,
                    operationId: options?.operationId,
                    toolCallId: options?.toolContext?.callId,
                    error:
            ...
            voltlogger.info('get_forecast_open_meteo: end', {
                tool: tool.name,
                operationId: options?.operationId,
                toolCallId: options?.toolContext?.callId,
            })
    ...
    execute: async (args, context?: ToolExecuteOptions) => {
        ...
        try {
            ...
            const geoResp = await http.get<NominatimResult[]>(nominatimUrl, {
                headers: {
                    'User-Agent':
            ...
            voltlogger.info(
                `get_forecast_open_meteo: fetching forecast for ${placeName} (${lat},${lon})`
            )
            ...
            const weatherResp = await http.get<unknown>(openMeteoUrl, {
                responseType: 'json',
            })
            ...
            voltlogger.error(
                `get_forecast_open_meteo failed for ${location}: ${formatError(error)}`
            )
            return {
                success: false,
                error: `Failed to fetch forecast for ${location}: ${formatError(error)}`,
                timestamp: new Date().toISOString(),
            }
        }
    },
...
export const weatherToolkit = createToolkit({
    tools: [getWeatherTool, getForecastOpenMeteo] as unknown as Tool[],
    name: 'weather_toolkit',
    description:
        'Tools for fetching weather information (wttr.in summary + Open-Meteo structured forecasts)',
})
