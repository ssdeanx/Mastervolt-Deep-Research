...
async function retrieveDocuments(
  ...
  try {
    ...
    const pointsRaw = ((Boolean(rawResponse)) && typeof rawResponse === 'object') ? (rawResponse as Record<string, unknown>)['points'] : undefined
    ...
    for (const rawPoint of pointsRaw) {
      if (!(rawPoint) || typeof rawPoint !== 'object') {
        // Avoid assigning unknown/any values directly to structured log fields.
        // Serialize the rawPoint safely for logging.
        let rawPointRepr: string
        try {
          rawPointRepr = typeof rawPoint === 'string' ? rawPoint : JSON.stringify(rawPoint)
        } catch {
          rawPointRepr = String(rawPoint)
        }
        voltlogger.debug('Skipping invalid point (not an object) in Qdrant results', { rawPoint: rawPointRepr })
        continue
      }
...
export const qRetriever = new QdrantRetriever(
  { toolName: 'QdrantRetriever', toolDescription: 'Retriever using Qdrant vector database', logger: voltlogger }
)
