...
`@voltagent/voice` exposes VoltAgent agents via JSON-RPC. `@voltagent/server-hono` wires the routes into the default Hono server (feel free to swap it out for another framework later).
...
// Trigger an action that emits event
try {
...
// Define your custom provider optionstype MyCustomProviderOptions = {
  apiKey: string;
  region?: string;
