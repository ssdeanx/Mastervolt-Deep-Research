...
```bash
curl \  -X POST "http://localhost:3141/a2a/support-agent?context=%7B%22userId%22%3A%22user-42%22%7D" \  -H "Content-Type: application/json" \  -d '{  "jsonrpc": "2.0",  "id": "req-1",  "method": "message/send",  "params": { "message": { "kind": "message", "role": "user", "messageId": "msg-1", "parts": [{ "kind": "text", "text": "Hello" }] } }}'
```

## Configure Which Agents Are Exposed (Optional)​

You can publish agents that are not part of the primary VoltAgent registry or filter the combined list before answering A2A requests:src/a2a/server.ts
...
By default, `A2AServer` uses an in-memory store. Provide your own implementation (Redis, Postgres, KV…) by implementing the `TaskStore` interface:src/a2a/redis-store.ts
