...
export function unflattenObject(obj: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const key in obj) {
    if (Object.prototype.hasOwnProperty.call(obj, key)) {
      ...
      if (value !== null && typeof value === "object" && !Array.isArray(value) && Object.keys(value as Record<string, unknown>).length === 0) {
...
export function detectDataFormat(data: string): FormatDetectionResult {
  ...
  // Check for XML
  if (format === "unknown" && (trimmed.startsWith("<?xml") || trimmed.startsWith("<"))) {
    ...
    if (hasClosingTags || hasXmlDeclaration) {
      ...
      if (hasXmlDeclaration) { indicators.push("XML declaration present") }
      if (hasClosingTags) { indicators.push("Closing tags found") }
    }
  }
  ...
  if (format === "unknown") {
    ...
    if (lines.length > 1) {
      ...
      if (firstLineCommas > 0 && firstLineCommas === secondLineCommas) {
        format = "csv"
        confidence = 0.8
        indicators.push("Consistent comma-separated structure", `${firstLineCommas + 1} columns detected`)
      }
    }
  }
  ...
  // Check for YAML
  if (format === "unknown") {
    const hasYamlIndicators = /^[\w-]+:\s*.+$/m.test(trimmed) || trimmed.includes("---")
...
export function convertFormat(
  ...
  try {
    ...
    if (toFormat === "json") {
      result = typeof jsObject === "string" ? jsObject : JSON.stringify(jsObject, null, 2)
...
export const normalizeDataTool = createTool({
  name: "normalize_data",
  description: "Flattens and normalizes nested data structures into a consistent format with dot notation keys.",
  parameters: z.object({
    data: z.record(z.string(), z.any()).describe("The nested data object to normalize"),
  ...
  execute: (args, context) => {
    ...
    try {
      voltlogger.info("Normalizing data structure", { operationId: context?.operationId })
      ...
      voltlogger.info("Data normalization complete", { operationId: context?.operationId })
      return result
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error)
      voltlogger.error(`Data normalization failed: ${errorMessage}`, { operationId: context?.operationId })
...
export const detectFormatTool = createTool({
  name: "detect_format",
  description: "Auto-detects the format of data string (JSON, CSV, XML, YAML) with confidence scoring.",
  parameters: z.object({
  ...
  execute: (args, context) => {
    ...
    try {
      voltlogger.info("Detecting data format", { operationId: context?.operationId })
      const result = detectDataFormat(args.data)
      voltlogger.info(`Format detected: ${result.format}`, { operationId: context?.operationId })
      return result
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error)
      voltlogger.error(`Format detection failed: ${errorMessage}`, { operationId: context?.operationId })
...
export const convertFormatTool = createTool({
  name: "convert_format",
  description: "Converts data between different formats (JSON, CSV, XML) with validation.",
  parameters: z.object({
    data: z.union([z.string(), z.array(z.any()), z.record(z.string(), z.any())]).describe("The data to convert"),
    fromFormat: z.enum(["json", "csv", "xml", "yaml", "unknown"]).describe("The source format"),
  ...
  execute: (args, context) => {
    ...
    try {
      voltlogger.info(`Converting from ${args.fromFormat} to ${args.toFormat}`, { operationId: context?.operationId })
      const result = convertFormat(args.data, args.fromFormat, args.toFormat)

      if (!result.success) {
        throw new Error(result.error || "Conversion failed")
      }

      voltlogger.info("Format conversion complete", { operationId: context?.operationId })
      return {
        data: result.data,
      ...
      const errorMessage = error instanceof Error ? error.message : String(error)
      voltlogger.error(`Format conversion failed: ${errorMessage}`, { operationId: context?.operationId })
...
export const validateSchemaTool = createTool({
  ...
  description: "Validates data against a schema definition with detailed error reporting.",
  parameters: z.object({
    data: z.any().describe("The data to validate"),
    schema: z.record(z.string(), z.object({
      type: z.enum(["string", "number", "boolean", "array", "object", "null", "any"]).describe("Expected type"),
      required: z.boolean().default(false).describe("Whether the field is required"),
      minLength: z.number().optional().describe("Minimum length for strings/arrays"),
      maxLength: z.number().optional().describe("Maximum length for strings/arrays"),
      min: z.number().optional().describe("Minimum value for numbers"),
      max: z.number().optional().describe("Maximum value for numbers"),
      pattern: z.string().optional().describe("Regex pattern for strings"),
  ...
  execute: (args, context) => {
    ...
    try {
      voltlogger.info("Validating data against schema", { operationId: context?.operationId })
      ...
      for (const [field, rules] of Object.entries(args.schema)) {
        ...
        if (rules.required && (value === undefined || value === null)) {
          errors.push({ path: field, message: "Required field is missing", expected: rules.type, received: "undefined" })
          continue
        }
        ...
        if (rules.type !== "any" && actualType !== rules.type) {
          errors.push({ path: field, message: "Type mismatch", expected: rules.type, received: actualType })
        }

        if (rules.type === "string" && typeof value === "string") {
          if (rules.minLength !== undefined && value.length < rules.minLength) {
            errors.push({ path: field, message: `String too short (min: ${rules.minLength})`, expected: `length >= ${rules.minLength}`, received: `length ${value.length}` })
          }
          if (rules.maxLength !== undefined && value.length > rules.maxLength) {
            errors.push({ path: field, message: `String too long (max: ${rules.maxLength})`, expected: `length <= ${rules.maxLength}`, received: `length ${value.length}` })
          }
          if (rules.pattern) {
            const regex = new RegExp(rules.pattern)
            if (!regex.test(value)) {
              errors.push({ path: field, message: "String does not match pattern", expected: rules.pattern, received: value })
            }
          }
        }
        ...
        if (rules.type === "number" && typeof value === "number") {
          if (rules.min !== undefined && value < rules.min) {
            errors.push({ path: field, message: `Number too small (min: ${rules.min})`, expected: `>= ${rules.min}`, received: String(value) })
          }
          if (rules.max !== undefined && value > rules.max) {
            errors.push({ path: field, message: `Number too large (max: ${rules.max})`, expected: `<= ${rules.max}`, received: String(value) })
          }
        }

        if (rules.type === "array" && Array.isArray(value)) {
          if (rules.minLength !== undefined && value.length < rules.minLength) {
            errors.push({ path: field, message: `Array too short (min: ${rules.minLength})`, expected: `length >= ${rules.minLength}`, received: `length ${value.length}` })
          }
          if (rules.maxLength !== undefined && value.length > rules.maxLength) {
            errors.push({ path: field, message: `Array too long (max: ${rules.maxLength})`, expected: `length <= ${rules.maxLength}`, received: `length ${value.length}` })
          }
        }
      }
      ...
      if (args.strict) {
        for (const key of Object.keys(dataObj)) {
          if (!(key in args.schema)) {
            warnings.push({ path: key, message: "Unknown field not in schema" })
          }
        }
      }

      const result: SchemaValidationResult = { valid: errors.length === 0, errors, warnings, timestamp: new Date().toISOString() }
      voltlogger.info(`Schema validation: ${result.valid ? "valid" : "invalid"}`, { operationId: context?.operationId })
      return result
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error)
      voltlogger.error(`Schema validation failed: ${errorMessage}`, { operationId: context?.operationId })
...
export const aggregateDataTool = createTool({
  name: "aggregate_data",
  description: "Groups data by a specified field and computes statistics (count, sum, average, min, max).",
  parameters: z.object({
    data: z.array(z.record(z.string(), z.any())).describe("Array of data objects to aggregate"),
  ...
  execute: (args, context) => {
    ...
    try {
      voltlogger.info(`Aggregating data by ${args.groupBy}`, { operationId: context?.operationId })
      ...
      for (const [key, items] of groupedData) {
        const group: DataAggregation["groups"][0] = { key, count: items.length }

        if (args.aggregateField) {
          const values = items
            .map(item => (item as Record<string, unknown>)[args.aggregateField!])
            .filter((v): v is number => typeof v === "number")

          if (values.length > 0) {
            if (args.computeStats.includes("sum")) { group.sum = values.reduce((a, b) => a + b, 0) }
            if (args.computeStats.includes("average")) { group.average = values.reduce((a, b) => a + b, 0) / values.length }
            if (args.computeStats.includes("min")) { group.min = Math.min(...values) }
            if (args.computeStats.includes("max")) { group.max = Math.max(...values) }
          }
        }
        groups.push(group)
      }
      ...
      const result: DataAggregation = {
        ...
        groups,
        statistics: { totalRecords: args.data.length, groupCount: groups.length, computedAt: new Date().toISOString() },
      }

      voltlogger.info(`Aggregation complete: ${groups.length} groups`, { operationId: context?.operationId })
      return result
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error)
      voltlogger.error(`Data aggregation failed: ${errorMessage}`, { operationId: context?.operationId })
...
export const cleanDataTool = createTool({
  name: "clean_data",
  description: "Cleans data by handling missing values, trimming strings, removing duplicates, and fixing inconsistencies.",
  parameters: z.object({
    data: z.array(z.record(z.string(), z.any())).describe("Array of data objects to clean"),
    removeNulls: z.boolean().default(true).describe("Remove records with null/undefined values"),
    trimStrings: z.boolean().default(true).describe("Trim whitespace from string values"),
    fillMissing: z.record(z.string(), z.any()).optional().describe("Default values to fill for missing fields"),
  ...
  execute: (args, context) => {
    ...
    try {
      voltlogger.info("Cleaning data", { operationId: context?.operationId })
          ...
          for (const key in newItem) {
            if (typeof newItem[key] === "string") {
              ...
              if (trimmed !== newItem[key]) {
                changes.push({ index, field: key, action: "trimmed", oldValue: newItem[key], newValue: trimmed })
                newItem[key] = trimmed
              }
            }
          }
      ...
      if (args.removeNulls) {
        ...
        cleaned = cleaned.filter((item, index) => {
          const hasNull = Object.values(item).some(v => v === null || v === undefined)
          if (hasNull) { changes.push({ index, field: "*", action: "removed" }) }
          return !hasNull
        })
        voltlogger.debug(`Removed ${beforeCount - cleaned.length} records with null values`)
      }
      ...
      if (args.removeDuplicates) {
        ...
        voltlogger.debug(`Removed ${beforeCount - cleaned.length} duplicate records`)
      }
      ...
      const result: CleanedData = {
        ...
        modifiedCount: changes.filter(c => c.action !== "removed").length,
        changes,
        timestamp: new Date().toISOString(),
      }

      voltlogger.info(`Data cleaning complete: ${result.removedCount} removed, ${result.modifiedCount} modified`, { operationId: context?.operationId })
      return result
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error)
      voltlogger.error(`Data cleaning failed: ${errorMessage}`, { operationId: context?.operationId })
...
export const dataProcessingToolkit: Toolkit = createToolkit({
  ...
  description: "Comprehensive data processing toolkit for transforming, validating, and normalizing various data formats.",
  instructions: `
...
- Clean data before analysis to ensure accurate statistics
  ...
  tools: [normalizeDataTool, detectFormatTool, convertFormatTool, validateSchemaTool, aggregateDataTool, cleanDataTool],
})
